...
<body class="  ">
                    ...
                    <div class="card">
                        <div class="card-header d-flex justify-content-between">
                            <div class="header-title">
                                <h4 class="card-title text-dark">Promocode List</h4>
                            </div>
                                ...
                                <table id="datatable" class="table table-striped" role="grid"
                                    ...
                                    <tbody id='record'>
                                        ...
                                        foreach ($promocodes as $promocode) {
                                            ...
                                            echo "<td>" . $promocode['promocode'] . "</td>";
                                            echo "<td>" . $promocode['discount'] . " %</td>";

                                            echo "<td>                                                                
                                                    <a class='btn btn-sm btn-icon btn-danger deletePromocode' data-bs-toggle='tooltip' data-bs-placement='top' title='Delete' href='#' data-promocode-id='" . $id . "'>
    ...
    <div id="addCategoryModal" class="modal addCategoryModal">
        <div class="modal-content">
            <h4 class="text-center fw-700 text-primary text-uppercase mb-2">Add Promocode</h4>
    ...
    <!-- <div id="editCategoryModal" class="modal editCategoryModal">
        <div class="modal-content">
            <h4 class="text-center fw-700 text-primary text-uppercase mb-2">Edit Promocode</h4>
            <form method="POST" id='editCategory'>
                <div class="row">
                    <div class="col-12">
                        <div class="form-group">
                            <label class="form-label" for="promocode">Promocode</label>
                            <input type="text" class="form-control" name="promocode" id="edit_promocode">
                        </div>
                    </div>
                    <div class="col-12">
                        <div class="form-group">
    ...
    <script>
        ...
        $(document).ready(function () {
            $('#addPromocode').submit(function (e) {
                ...
                $.ajax({
                    type: 'POST',
                    url: 'promocodebackend.php',
                    data: formData,
                    success: function (response) {
                        if (response.trim() === "Promocode added successfully!") {
                            console.log(response);
                            alert(response);
        ...
        // add category - end
        // edit category - start
        // $(document).on('click', '.editPromocodeBtn', function () {
        //     var promocodeId = $(this).data('promocode-id');
        //     // Fetch category details using AJAX
        //     $.ajax({
        //         type: 'POST',
        //         url: 'promocodebackend.php',
        //         data: { req_type: 'get_promocode', promocode_id: promocodeId },
        ...
        //                 $('#promocode_id').val(promocodeId);
        //                 $('#edit_promocode').val(response.promocode.promocode);
        //                 $('#edit_discount').val(response.promocode.discount);
        //                 // Open the edit modal
            ...
            $(document).on('click', '.deletePromocode', function (e) {
                ...
                if (confirm("Are you sure you want to delete this promocode?")) {
                    $.ajax({
                        type: 'POST',
                        url: 'promocodebackend.php',
                        data: { req_type: 'delete', promocode_id: promocodeId },
                        success: function (response) {
                            console.log(response); // Log the response to the console
                            if (response.trim() === "Promocode deleted successfully!") {
                                console.log("Promocode deleted successfully");
                                alert(response);
                                window.location.href = 'promocode.php';
                            } else {
                                console.log("Error deleting promocode: " + response.trim());
                            }
                        },
                        error: function (error) {
                            console.log(error);
